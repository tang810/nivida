...
from alpha.actions import Action, ActionOutput
from alpha.actions.action_node import ActionNode
from alpha.actions.add_requirement import UserRequirement
from alpha.context_mixin import ContextMixin
from alpha.logs import logger
from alpha.memory import Memory
from alpha.provider import HumanProvider
from alpha.schema import Message, MessageQueue, SerializationMixin
from alpha.strategy.planner import Planner
from alpha.utils.common import any_to_name, any_to_str, role_raise_decorator
from alpha.utils.project_repo import ProjectRepo
from alpha.utils.repair_llm_raw_output import extract_state_value_from_output

if TYPE_CHECKING:
    from alpha.environment import Environment  # noqa: F401
...
class RoleContext(BaseModel):
    ...
    def model_rebuild(cls, **kwargs):
        from alpha.environment.base_env import Environment  # noqa: F401
...
class Role(SerializationMixin, ContextMixin, BaseModel):
    ...
    async def _act(self) -> Message:
        ...
        init(autoreset=True)
        print()
        print(Fore.GREEN + f"{self._setting}" + Fore.RESET ,end='')
        print(" 正在做 : ",end='')
        print(Fore.YELLOW + f"{self.rc.todo.desc}" + Fore.RESET)
        print()
